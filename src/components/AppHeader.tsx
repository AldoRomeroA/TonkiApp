  ...
  return (
    <header className="sticky top-0 z-40 border-b border-tonki-border bg-tonki-canvas/90 backdrop-blur-md">
      <div className="mx-auto flex w-full max-w-[600px] items-center justify-between gap-4 px-4 py-3 sm:px-6">
        <Link
          href="/dashboard"
          className="flex shrink-0 items-center gap-2.5 text-tonki-text transition-colors hover:text-tonki-accent"
        >
          <Image src="/logo.png" alt="Tonki" width={32} height={32} priority />
          <span className="text-lg font-bold tracking-tight">Tonki</span>
        </Link>

        {title ? (
          <h1 className="min-w-0 flex-1 truncate text-center text-[15px] font-bold leading-snug text-tonki-text sm:text-[17px]">
            {title}
          </h1>
        ) : null}

        {right ?? (
          <AccountSlideOverTrigger className="min-w-0 max-w-[min(200px,45vw)] shrink-0 truncate text-right text-sm font-medium text-tonki-text-muted transition-colors hover:text-tonki-text-secondary sm:text-base" />
        )}
      </div>
    </header>
  );
